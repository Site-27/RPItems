...
public class HeavyTranq : CustomWeapon
{
    ...
    private float HumanDuration { get; set; } = 150f;
    ...
    protected override void OnHurting(HurtingEventArgs ev)
    {
        ...
        ev.Amount = Plugin.Instance.Config.HeavyTranqDamage;

        if (ev.Player.IsDead) return;
        if (!ev.Player.IsConnected) return;
        
        
        if (target.IsScp) // PATH 1: Target is an SCP
        {
        ...
        // PATH 2: Target is a non-SCP role
        ...
        var arrestChance = _rnd.Next(1, 5); // camel case dictates that local/private variables should be word1Word2Word3 and so on - first letter lowercase
        if (arrestChance == 1)
        {
